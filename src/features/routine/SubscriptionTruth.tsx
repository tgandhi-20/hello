        ...
        {KNOWN_SUBSCRIPTIONS.map((s) => (
          <li key={s.id} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
            <span className="text-sm text-ink-1">
              {s.merchant}
              {s.id === 'netflix' ? <span className="text-ink-3"> (your half, split)</span> : null}
            </span>
            <span className="money text-sm text-ink-1">{formatMoney(s.amountCents)}</span>
          </li>
        ))}
      </ul>
      <div className="flex items-center justify-between border-t border-hairline pt-2">
        <span className="text-sm font-semibold text-ink-1">Known total</span>
        <span className="money text-sm text-ink-1">{formatMoney(KNOWN_SUBSCRIPTIONS_TOTAL_CENTS)}</span>
      </div>

      <p className="text-xs text-ink-3">Cancelled, not billed: {CANCELLED_SUBSCRIPTIONS.join(', ')}.</p>
      <p className="text-xs text-ink-3">
        An earlier estimate put subscriptions at{' '}
        <span className="money text-ink-3">{formatMoney(MISLEADING_PRIOR_SUBSCRIPTIONS_FIGURE_CENTS)}</span>/month.
      ...
      {unknown.length > 0 ? (
        <div className="flex flex-col gap-2 rounded-card bg-[color-mix(in_srgb,var(--caution)_12%,transparent)] px-3 py-3">
          ...
          <ul className="flex flex-col gap-1.5">
            {unknown.map((s) => (
              <li key={s.id} className="flex items-center justify-between text-sm text-ink-1">
                <span className="truncate">{s.merchant}</span>
                <span className="money shrink-0 text-ink-1">
