        ...
        <div className="grid grid-cols-3 gap-3">
          {categories.map((c) => (
            <button
              ...
              className="flex min-h-[80px] flex-col items-center justify-center gap-2 rounded-2xl border border-border bg-surface-2 px-2 py-2 text-center active:scale-[0.97] transition-transform duration-200"
            >
              <CategoryIcon icon={c.icon} colorToken={c.colorToken} size="sm" />
              <span className="line-clamp-1 text-xs font-medium text-text-1">{c.label}</span>
            </button>
          ))}
        </div>
      </div>
